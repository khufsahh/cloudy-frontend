...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchCheckIns = async () => {
      try {
        const response = await fetch('https://cloudy-check-in-production.up.railway.app/api/checkins/send', {
        ...
        <button
          onClick={async () => {
            try {
              ...
              const response = await fetch('http://localhost:5000/api/checkins/send', {
                method: 'POST',
